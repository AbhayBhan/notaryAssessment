...
const MobNotaryForm = () => {
  ...
  const [quote, setQuote] = useState(costingdetails.GEN_ONLINE.baseNotarisationCost_RON);
  const [signatures, SetSignatures] = useState(0);
  const [extraWitness, setExtraWitness] = useState(0); 
  const [extraSigners, setExtraSigners] = useState(0);

  useEffect(() => {
    const baseCost = costingdetails.GEN_ONLINE.baseNotarisationCost_RON
    const extraSignCost = costingdetails.GEN_ONLINE.one_signatureCost_RON;
    const extraWitnessCost = costingdetails.GEN_ONLINE.extraWitnessCost_RON;
    const extraSignersCost = costingdetails.GEN_ONLINE.extraSignersCost_RON;
